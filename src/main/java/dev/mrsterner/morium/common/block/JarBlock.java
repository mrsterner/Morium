    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            if(world.getBlockEntity(pos) instanceof JarBlockEntity jarBlockEntity){
                ...
                if(MoriumTags.JAR.contains(player.getStackInHand(hand).getItem()) && (jarBlockEntity.getStack(0).isEmpty() || jarBlockEntity.getStack(0).getItem().equals(player.getStackInHand(hand).getItem()))){
